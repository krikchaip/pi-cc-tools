...
const viewport = { columns: 180, rows: 40 } as const;
const family = singleEditFamily(viewport);

export default family.scenario({
  name: "split single Edit opens directly after its summary",
  start: {
    mode: "no-session",
  ...
  async run(terminal) {
    assertOpeningAdjacent(await captureSingleEdit(terminal, viewport), "split");
  },
});
