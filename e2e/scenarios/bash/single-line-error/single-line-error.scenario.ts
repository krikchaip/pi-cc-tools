...
const command = "printf '%s%s\\n' BASH_ FAILURE_OUTPUT >&2; exit 7";

export default bashFamily.scenario({
  name: "collapsed single-line Bash errors show the command once",
  start: {
    mode: "session",
    session: {
      path: fixture("single-line-error/bash-single-line-error-session.jsonl"),
      replaceCwd: "/tmp/pi-cc-tools-bash-single-line-error",
    },
