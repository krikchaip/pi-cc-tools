...
The public runtime interface is limited to `state(target)`, `declare(target, copy, options)`, `publish(owner, finalRows)`, and `installClickExpansion({ enabled })`. `extensions/pi-host/grouping.ts` and `extensions/pi-host/transcript.ts` supply narrow host adapters for presentation layout, activation transactions, rollback, and retained Pi prototypes; `extensions/pi-host/click-routing.ts` owns shared dispatch and invalidation. Runtime state and host bridges use `Symbol.for(...)` roots so extension reloads keep one current implementation.
